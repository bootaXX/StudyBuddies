...
local function create_calendar( month, year )
	...
	-- Background
	local rect = display.newRect(group, -pad*0.5, 120, _W+pad, 290)
    ...
    local monthName = display.newText(group, monthStr, 270, 80, native.systemFont, 32)
    monthName.anchorX, monthName.anchorY = 0, 0
    monthName:setFillColor( 255, 255, 255 )
    ...
    for i = 1, rows do
        ...
        for j = 1, 7 do
        	...
        	-- Day text
            local day = display.newText(group, nDay, x+145, y+170, native.systemFont, 20)
            day.anchorX, day.anchorY = 0.5, 0
            if selMonth ~= 'current' then
            	day:setFillColor(255, 0, 0)
            else
            	day:setFillColor(0, 0, 255 )
            end
            ...
            if i == 1 then
                ...
                local weekday = display.newText(group, str, x+145, y+137, native.systemFont, 20)
            ...
            if nDay == date.day and nMonth == date.month and nYear == date.year then 
                local circle = display.newCircle(group, x+145, y+160, 20)
